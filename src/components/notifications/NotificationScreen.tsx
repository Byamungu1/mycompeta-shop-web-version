...
const CATEGORY_CONFIG: Record<string, {
    ...
    promo: {
        icon: <Tag size={16} className="text-market-500" />,
        bgClass: 'bg-market-100',
    },
    account: {
        icon: <User size={16} className='text-brand-500' />,
        bgClass: 'bg-brand-100',
    },
    system: {
        icon: <User size={16} className='text-sand-600' />,
        bgClass: 'bg-sand-200',
    },
};
...
const NotificationItem = ({
    ...
    const category = item.notif_type.includes('order') ? 'order_placed' : item.notif_type;
    ...
    return (
        <TouchableOpacity
            ...
            activeOpacity={0.75}
            className={`flex flex-row border-b border-sand-400 border-1 items-start px-4 py-4 ${!item.is_read ? 'bg-brand-500/10' : 'bg-sand-100'}`}
            style={{ gap: 12 }}>
...
const NotificationsScreen = ({
    ...
    const handleMarkRead = async (notification: Notification) => {
        ...
        router.push({
            pathname: "/(root)/notification/[id]",
            params: {
                id: notification.id,
                notificationData: encodeURIComponent(JSON.stringify(notification)),
            },
        });
    ...
    const handleRefresh = async () => {
        ...
        await refetch(null);
        await refetchNotifications(notif_type);
    ...
    return (
        <SafeAreadiv className='bg-sand-100 flex-1'>
            <div className='flex flex-row items-center justify-between gap-3 px-4 sm:px-6 lg:px-8 pt-4 pb-2 w-full max-w-3xl mx-auto'>
                <div style={{ gap: 2, flex: 1 }}>
                    <Header
                        title={headerTitle}
                        subtitle={`${counts.unreadNotifications} unread`}
            ...
            {notificationLoading ? (
                ...
                <SectionList
                    ...
                    contentContainerStyle={{ paddingBottom: 120, gap: 2, paddingTop: 16 }}
